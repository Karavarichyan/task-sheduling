...
import { useSaveTeamMemberMutation } from '@/data/mutations/useSaveTeamMemberMutation'
import { useUpdateTeamMemberMutation } from '@/data/mutations/useUpdateTeamMemberMutation'
...
const { mutate: saveMember, isPending: isAdding } = useSaveTeamMemberMutation();
const { mutate: updateMember, isPending: isUpdating } = useUpdateTeamMemberMutation();

const isEditMode = computed(() => !!props.initialData);
const dialogTitle = computed(() => isEditMode.value ? 'Edit Team Member' : 'Add New Team Member');
const dialogDescription = computed(() => isEditMode.value ? 'Update the details for this team member.' : 'Fill in the details to add a new team member.');
const isSubmitting = computed(() => isAdding.value || isUpdating.value);
const submitButtonText = computed(() => {
    if (isSubmitting.value) {
        return isEditMode.value ? 'Updating Member...' : 'Adding Member...';
    }
    return isEditMode.value ? 'Update Team Member' : 'Add Team Member';
});
const isFormValid = computed(() => {
    return (
...
watch(() => [props.initialData, props.isOpen], ([newInitialData, newIsOpen]) => {
...
const handleSubmit = () => {
    ...
    const mutationHandler = isEditMode.value
        ? updateMember
        : saveMember;
    ...
    mutationHandler(dataToSend, {
        ...
        onError: (error: any) => {
            console.error(`Failed to ${isEditMode.value ? 'update' : 'save'} team member:`, error);
        },
    });
}
</script>
